...
/// struct for typed errors of method [`directory_forms_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DirectoryFormsPostError {
    Status403(),
    Status401(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`directory_organizations_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DirectoryOrganizationsPostError {
    Status403(),
    Status401(),
    UnknownValue(serde_json::Value),
}
...
pub async fn directory_forms_post(configuration: &configuration::Configuration, page_size: Option<i32>, continuation_token: Option<&str>, hello_asso_api_v5_common_models_directory_list_forms_request: Option<models::HelloAssoApiV5CommonModelsDirectoryListFormsRequest>) -> Result<models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableFormModel, Error<DirectoryFormsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_page_size = page_size;
    let p_query_continuation_token = continuation_token;
    let p_body_hello_asso_api_v5_common_models_directory_list_forms_request = hello_asso_api_v5_common_models_directory_list_forms_request;

    let uri_str = format!("{}/directory/forms", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_hello_asso_api_v5_common_models_directory_list_forms_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableFormModel`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableFormModel`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DirectoryFormsPostError> = serde_json::from_str(&content).ok();
...
pub async fn directory_organizations_post(configuration: &configuration::Configuration, page_size: Option<i32>, continuation_token: Option<&str>, hello_asso_api_v5_common_models_directory_list_organizations_request: Option<models::HelloAssoApiV5CommonModelsDirectoryListOrganizationsRequest>) -> Result<models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableOrganizationModel, Error<DirectoryOrganizationsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_page_size = page_size;
    let p_query_continuation_token = continuation_token;
    let p_body_hello_asso_api_v5_common_models_directory_list_organizations_request = hello_asso_api_v5_common_models_directory_list_organizations_request;

    let uri_str = format!("{}/directory/organizations", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = req_builder.json(&p_body_hello_asso_api_v5_common_models_directory_list_organizations_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableOrganizationModel`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::HelloAssoApiV5CommonModelsCommonResultsWithPaginationModelSynchronizableOrganizationModel`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DirectoryOrganizationsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
